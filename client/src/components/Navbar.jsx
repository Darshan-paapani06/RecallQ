    ...
    const renderRoleInfo = () => {
        ...
        return ( <
                ...
                userData.organization ? .name && ( <
                    >
                    { " | " } <
                    span className = "font-semibold text-blue-700" > { userData.organization.name.toUpperCase() } <
                    /span> < /
                    >
                )
            } <
...
return ( <
    ...
    div className = "flex items-center gap-4" > {
        userData ? ( <
            ...
            aria - label = "Open user menu" > { userData ? .name ? .charAt(0) ? .toUpperCase() ? ? "U" } <
            /button> { menuOpen && renderDropdown() } < /
            div > <
            />
        ) : ( <
            button onClick = {
